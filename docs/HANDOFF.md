...
| **开始菜单 Win10 布局**：左侧 48px 窄栏（#171717）——顶部 ☰ 汉堡（展开/折叠 200px）、底部功能区（账户→设置/文档/图片→电源最底）；电源弹关机/重启/睡眠菜单（与侧栏等宽，MVP 动作占位）；主区磁贴 | 与 Win10 UI 对齐（渲染验证：侧栏 201 色、电源区 132 色含白色电源图标）；账户/设置动作与真实关机为后续里程碑（PAM/systemctl） |
