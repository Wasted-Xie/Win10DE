...
| **开始按钮 = 发行版 logo**（`/usr/share/pixmaps/archlinux-logo.svg`，缺失回退文字） | 系统发行版图标替代"开始"文字（Arch 蓝 #1793D1 实测渲染）；按钮 48×48 正方形（1:1 与任务栏同高）、图标固定 48×48、`padding:0` 贴左（按钮左缘 x=0 实测；图标可见 42×42 为 SVG 画布留白） |
