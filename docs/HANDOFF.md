...
| **磁贴四种尺寸**（`TileButton`+`FlowLayout`，右键菜单自由设置）：小 48×48 / 中 96×96（默认）/ 大 192×192 / 宽 192×96；每行 3 个中磁贴（3×96=288 无缝，实测） | Win10 磁贴大小；流式排布自动换行；**FlowLayout 宽度取父 widget 实际宽度**（setGeometry rect 在 layer-shell 时序中 100↔288 不稳，按 rect 排布错误换行——真实运行验证）；尺寸变化触发重排 |
